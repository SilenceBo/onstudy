...
<body class="gray-bg">
							...
							<div class="example">
								...
								<div class="btn-group hidden-xs" id="exampleTableEventsToolbar"
									...
									<button type="button" class="btn btn-outline btn-default"
										data-toggle="modal" onclick='javascript:window.location.href="${APP_PATH }/question/findAll.action"'>
								...
								<table id="exampleTableEvents" data-height="800"
									...
									<c:forEach items="${anList }" var="answer">
										<tr id="${answer.aid }" class="tr" ids="123">
											<td><input id="ids" name="ids" type="checkbox"
												class="noborder" value="${answer.aid }"></td>
											<td>${answer.qid }</td>
											<td>${answer.contents }</td>
											<td>${answer.goodanswer }</td>
											<td>${answer.yid==null?"无":answer.yid }</td>
											<td>${answer.creattime }</td>
											<c:choose>
	...
	<script type="text/javascript">
		...
		function deleteCarousel(del) {
			...
			var msg = "您确定要删除吗？";
			if (confirm(msg) == true) {
				window.location = "deleteAnswer.action?ids=" + id;
			}
		}
		...
		function check() {
			...
			if (confirm(msg) == true) {
				...
				if (becheckbox.length > 0) { //如果获取到
					...
					window.location = "deleteAnswer.action?ids=" + becheckbox;
					return false;
				} else {
					return false;
				}
